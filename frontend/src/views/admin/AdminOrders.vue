    ...
    <b-container >
        ...
        <b-row class="justify-content-center">
                ...
                <b-card v-for="order in this.orders" :bg-variant="getColor(order)" :header="getTitle(order)" class="text-center mt-3 mr-3"
                    ...
                    <b-button v-if="!order.sent_date" v-on:click="markAsSent(order)" class="mr-2 ml-2" variant="outline-success">
                        Marcar como enviado
...
export default {
    ...
    methods: {
        refreshOrders(){
            const API_URL = 'http://localhost/api/v1/orders'
        ...
        markAsSent(order){
            ...
            axios.get(API_URL+'/'+order.id+'/send' , {headers: {'Authorization': 'Bearer '+this.$store.state.auth.user.token} })
                .then(response => {
                    this.orders = response.data
                    this.refreshOrders();
                })
